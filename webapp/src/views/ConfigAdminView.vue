<template>
    <BasePage :title="$t('configadmin.ConfigManagement')" :isLoading="loading">
        ...
        <CardElement :text="$t('configadmin.ResetHeader')" textVariant="text-bg-primary" center-content add-space>
            <button
                class="btn btn-danger" @click="onFactoryResetModal">{{ $t('configadmin.FactoryResetButton') }}
            </button>

            <div class="alert alert-danger mt-3" role="alert" v-html="$t('configadmin.ResetHint')"></div>
        </CardElement>

        <ModalDialog modalId="factoryReset" small :title="$t('configadmin.FactoryReset')" :closeText="$t('configadmin.Cancel')"> {{ $t('configadmin.ResetMsg') }}
            <template #footer>
                <button type="button" class="btn btn-danger" @click="onFactoryResetPerform"> {{ $t('configadmin.ResetConfirm') }}</button>
            </template>
        </ModalDialog>
    </BasePage>
</template>
