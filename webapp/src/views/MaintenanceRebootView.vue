<template>
    <BasePage :title="$t('maintenancereboot.DeviceReboot')" :isLoading="dataLoading">
        ...
        <CardElement :text="$t('maintenancereboot.PerformReboot')" textVariant="text-bg-primary" center-content>
            <button
                class="btn btn-danger" @click="onOpenModal(performReboot)">{{ $t('maintenancereboot.Reboot') }}
            </button>

            <div class="alert alert-danger mt-3" role="alert" v-html="$t('maintenancereboot.RebootHint')"></div>
        </CardElement>

        <ModalDialog modalId="performReboot" small :title="$t('maintenancereboot.RebootAPP')" :closeText="$t('maintenancereboot.Cancel')"> {{ $t('maintenancereboot.RebootQuestion') }}
            <template #footer>
                <button type="button" class="btn btn-danger" @click="onReboot"> {{ $t('maintenancereboot.Reboot') }} </button>
            </template>
        </ModalDialog>
    </BasePage>
</template>
