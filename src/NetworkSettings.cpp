...
{
    if (_adminEnabled == true)
    {
        WiFi.mode(WIFI_AP_STA);
        String ssidString = getApName();
        String passwordString = Configuration.get().Security.Password;
        WiFi.softAPConfig(_apIp, _apIp, _apNetmask);
        WiFi.softAP(ssidString.c_str(), passwordString.c_str());
...
void NetworkSettingsClass::loop()
{
    ...
    {
        if (_adminEnabled == true && _adminTimeoutCounterMax > 0)
        {
...
void NetworkSettingsClass::applyConfig()
{
    ...
    if (strcmp(WiFi.SSID().c_str(), Configuration.get().WiFi.Ssid) || strcmp(WiFi.psk().c_str(), Configuration.get().WiFi.Password))
    {
        ...
        WiFi.begin(Configuration.get().WiFi.Ssid, Configuration.get().WiFi.Password, WIFI_ALL_CHANNEL_SCAN);
    } else
    {
...
void NetworkSettingsClass::setStaticIp()
{
    if (_networkMode == network_mode::WiFi)
    {
        if (Configuration.get().WiFi.Dhcp == true)
         {
