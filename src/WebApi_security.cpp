...
{
    if (WebApi.checkCredentials(request) == false)
    {
        return;
    }
...
void WebApiSecurityClass::onSecurityPost(AsyncWebServerRequest* request)
{
    ...
    if (root["password"].as<String>().length() < 6 || root["password"].as<String>().length() > WIFI_MAX_PASSWORD_STRLEN) {
        retMsg["message"] = "Password must between 6 and " STR(WIFI_MAX_PASSWORD_STRLEN) " characters long!";
